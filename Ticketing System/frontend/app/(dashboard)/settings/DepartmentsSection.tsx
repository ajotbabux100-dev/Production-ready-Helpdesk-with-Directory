...
export function DepartmentsSection() {
  const isAdmin = useHasPerm('departments', 'edit')
  ...
  const loadAgents = async () => {
    ...
    setAgents(all.filter((u) => u.role_detail?.is_super || u.role_detail?.permissions.includes('tickets.claim')))
  }
  ...
  const agentOptions = agents.map((u) => ({ value: u.id, label: `${u.full_name} (${u.role_detail?.name})` }))
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">{departments.length} departments configured</p>
            </div>
